function [varargout] = gp_new(hyp, inf, mean, cov, lik, x, y, xs, ys)


%%Modified code from GPML to give the full covariance as output (ys2,fs2).
...
%   ys2      predictive output covariance matrix
%   fmu      column vector (of length ns) of predictive latent means
%   fs2      predictive latent covariance matrix
...
%Change
    fs2_complete = zeros(ns,ns);
    ys2_complete = zeros(ns,ns);
    KSS_complete = feval(cov{:}, hyp.cov, xs(:,:)); 
    ymu_complete = zeros(ns,1);
    lp_complete = zeros(ns,1);
    fmu_complete = zeros(ns,1);
    if strcmp(cstr,'covFITC')                                % cross-covariances
      Ks_complete = feval(cov{:}, hyp.cov, x, xs(:,:)); Ks_complete = Ks_complete(nz,:); % res indep. of x
    else
      Ks_complete = feval(cov{:}, hyp.cov, x(nz,:), xs(:,:));        % avoid computation
    end
    ms_complete = feval(mean{:}, hyp.mean, xs(:,:));
    N_ = size(alpha,2);  % number of alphas (usually 1; more in case of sampling)
    Fmu_complete = repmat(ms_complete,1,N_) + Ks_complete'*full(alpha(nz,:));        % conditional mean fs|f
    fmu_complete(:) = sum(Fmu_complete,2)/N_;                                   % predictive means

    if Lchol    % L contains chol decomp => use Cholesky parameters (alpha,sW,L)
      V_complete  = L'\(repmat(sW,1,ns).*Ks_complete);
      %fs2(id) = kss - sum(V.*V,1)';                       % predictive variances
      fs2_complete = KSS_complete - (V_complete' * V_complete);
    else                % L is not triangular => use alternative parametrisation
      if isnumeric(L), LKs_complete = L*Ks_complete; else LKs_complete = L(Ks_complete); end    % matrix or callback
      %fs2(id) = kss + sum(Ks.*LKs,1)';                    % predictive variances
     % size(LKs_complete)
     % size(Ks_complete)
      fs2_complete = KSS_complete + LKs_complete' * Ks_complete;
    end
    fs2_complete = max(fs2_complete,0);   % remove numerical noise i.e. negative variances
    Fs2_comlete = repmat(fs2_complete,1,N_);     % we have multiple values in case of sampling
    if nargin<9
   %     size(Fs2_comlete)
   %     N_
      [Lp, Ymu, Ys2] = feval(lik{:},hyp.lik,[],   Fmu_complete(:),diag(Fs2_comlete));
    else %Does not run for our application.
      Ys = repmat(ys(:),1,N_);
      [Lp, Ymu, Ys2] = feval(lik{:},hyp.lik,Ys(:),Fmu(:),diag(Fs2_comlete));
    end
    lp_complete(:)  = sum(reshape(Lp, [],N_),2)/N_;    % log probability; sample averaging
    ymu_complete(:) = sum(reshape(Ymu,[],N_),2)/N_;          % predictive mean ys|y and ..
    
    ys2_complete(:) = Fs2_comlete + diag(sum(reshape(Ys2,[],N_),2)/N_) - diag(diag(Fs2_comlete));  % .. variance
   % nact = id(end);          % set counter to index of last processed data point
%
  if nargin<9
    varargout = {ymu_complete, ys2_complete, fmu_complete, fs2_complete, [], post};        % assign output arguments
  else % does not run for us
    varargout = {ymu_complete, ys2_complete, fmu_complete, fs2_complete, lp_complete, post};
  end
end
